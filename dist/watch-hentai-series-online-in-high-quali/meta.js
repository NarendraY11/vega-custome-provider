...
function absolute(value, pageUrl) { try { return new URL(value, pageUrl).href; } catch { return ""; } }
function text(value) { return value.replace(/\s+/g, " ").trim(); }
function jsonLd(html) { const out = []; const re = /<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi; let m; while ((m = re.exec(html))) { try { const v = JSON.parse(m[1].trim()); if (Array.isArray(v)) out.push(...v); else if (v && Array.isArray(v["@graph"])) out.push(...v["@graph"]); else out.push(v); } catch {} } return out; }
const getMeta = async function ({ link, providerContext }) {
  ...
  const parse = (source) => {
    ...
    const tags = Array.isArray(media.genre) ? media.genre.map(String) : media.genre ? [String(media.genre)] : []; const cast = Array.isArray(media.actor) ? media.actor.map((x) => String(x?.name || x)).filter(Boolean) : [];
    const anchors = $("a[href]").map((_, e) => ({ title: text($(e).attr("title") || $(e).text()), link: absolute($(e).attr("href") || "", link) })).get().filter((x) => x.title && x.link);
    const seasons = anchors.filter((x) => /\bseason\s*\d+\b/i.test(x.title) || /\/season(?:[-_\/] |\d)/i.test(x.link)).slice(0, 30);
    const episodes = anchors.filter((x) => /\b(?:episode|ep|e)\s*\d+\b/i.test(x.title) || /\/(?:episode|ep)(?:[-_\/] |\d)/i.test(x.link)).slice(0, 80);
    const directLinks = episodes.map((x) => ({ title: x.title, link: x.link, type: "series" })); const linkList = seasons.map((x) => ({ title: x.title, episodesLink: x.link })); if (!linkList.length && directLinks.length) linkList.push({ title: "Episodes", directLinks });
