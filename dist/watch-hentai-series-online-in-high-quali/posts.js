...
function parsePosts(html, pageUrl, cheerio) {
  const $ = cheerio.load(html); const out = []; const seen = new Set();
  const add = (title, link, image) => { title = text(title); if (!link || !image || !title || title.length < 2 || title.length > 180 || link === pageUrl || seen.has(link)) return; seen.add(link); out.push({ title, link, image }); };
...
async function fetchHtml(url, providerContext) {
...
function pageUrl(url, page) { if (page <= 1) return url; try { const u = new URL(url); if (/\/page\/\d+\/?$/i.test(u.pathname)) u.pathname = u.pathname.replace(/\/page\/\d+\/?$/i, "/page/" + page + "/"); else u.searchParams.set("page", String(page)); return u.href; } catch { return url; } }

const getPosts = async function ({ filter, page, providerContext }) {
